...
async def list_tools(
    ...
    query = select(Tool).where(Tool.is_published)
    count_query = select(func.count(Tool.id)).where(Tool.is_published)
...
async def get_tool(slug: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Tool).where(Tool.slug == slug, Tool.is_published)
    )
